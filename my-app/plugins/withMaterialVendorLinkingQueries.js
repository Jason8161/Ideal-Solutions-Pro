...
/** Adds Android 11+ package visibility for material vendor canOpenURL + package launch. */
function withMaterialVendorLinkingQueries(config) {
  return withAndroidManifest(config, (mod) => {
    const manifest = mod.modResults;
    if (!manifest.manifest) return mod;

    const additions = [
      ...MATERIAL_VENDOR_ANDROID_PACKAGES.map(packageQueryEntry),
      ...MATERIAL_VENDOR_ANDROID_SCHEMES.map(viewSchemeQueryEntry),
    ];
...
module.exports = withMaterialVendorLinkingQueries;
